...
from .permissions import IsModerator, IsAuthenticated, IsBuyer, IsRemoteService
...
def update_price(request, forecast_id):
    if not Forecast.objects.filter(pk=forecast_id).exists():
        return Response(status=status.HTTP_404_NOT_FOUND)

    forecast = Forecast.objects.get(pk=forecast_id)
    forecast.price = request.data.get("value")
    forecast.save()
...
def calculate_price(forecast_id):
    data = {
        "forecast_id": forecast_id
    }

    requests.post("http://remote_service:8080/calculate_price/", json=data, timeout=3)
